class Invader3 {
  ...
  Invader3(float startingPosX, float startingPosY){
    xpos = startingPosX;
    ypos = startingPosY;
    invaderColor = color(50, 255, 50);  // Bright green color
  ...
  void explode(Projectile projectile){
    // Only allow damage if current target type is 3 (Invader3)
    if(currentTargetType == 3 && projectile.xpos <= xpos+INVADER3_WIDTH/2 && projectile.xpos >= xpos-INVADER3_WIDTH/2 && projectile.ypos >= ypos-INVADER3_HEIGHT/2 && projectile.ypos <= ypos+INVADER3_HEIGHT/2){
    exploded = true;
    invaderDeath.play();
    invaderDeath.rewind();
