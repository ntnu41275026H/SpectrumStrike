  ...
  void display(){
    ...
    if(newHighScore==true){
      ...
      if(initialsEntered==true){
        ...
        saveStrings("InvadersHighScoreHolders.txt", scoreHolders);
        for(int i=0;i<highScoresStrings.length;i++){
          highScoresStrings[i] = "" + topHighScores[i];}
          saveStrings("InvadersHighScores.txt",highScoresStrings);
          lastScore = score;
          score = 0;
          initialsEntered = false;
          newHighScore=false;}}
    ...
    void insertScore(int[] highScoreList,String[] scoreHolders, int score){
      ...
      // Find the position where the new score should be inserted (array is sorted low to high)
      for(int i = 0; i < highScoreList.length; i++){
        if(score > highScoreList[i]){
          placeForHighScore = i;
        }
      }
      // Shift lower scores down (removing the lowest score at index 0)
      for(int i = 0; i < placeForHighScore; i++){
