class Invader2 {
  ...
  Invader2(float startingPosX, float startingPosY){
    xpos = startingPosX;
    ypos = startingPosY;
    invaderColor = color(0, 191, 255);  // Cyan/light blue color
  ...
  void draw() {
    if(killed==false){
      if(timeOfExplosion < 0){
       ...
       stroke(invaderColor);
       strokeWeight(3);
       ellipse(xpos, ypos+7, 0.7*INVADER2_WIDTH, 0.7*INVADER2_HEIGHT);
       ...
       fill(255,100,0);  // Orange eyes to stand out on cyan
  ...
  void explode(Projectile projectile){
    // Allow damage if current target type is 2 (Invader2) OR if player has Strong Bullets power-up
    if((currentTargetType == 2 || currentPowerUp == 4) && projectile.xpos <= xpos+0.7*INVADER2_WIDTH && projectile.xpos >= xpos-0.7*INVADER2_WIDTH && projectile.ypos >= ypos+7-0.7*INVADER2_HEIGHT && projectile.ypos <= ypos+7+0.7*INVADER2_HEIGHT){
      exploded = true;
      invaderDeath.play();
      invaderDeath.rewind();
